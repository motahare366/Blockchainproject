...
//400000000000000000
    ...
    if(SIC!=0){
        ...
        for(uint i=0;i<Eui.length;i++)
      {
            ...
            // SDSqr=Eus-reputationAverage;["0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2","0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db","0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB"] 
            // SDSub+=SDSqr**2;
    edgeserverArray.push(edgeserver);
    mobileuserArray.push(mobileuser);
    reputation[edgeserver][mobileuser][id]=Eus;
   ...
   function IndirectReputation(address[]memory _addressuser ,address edgeserver,address mobileuser)public  {
   SD59x18 reputationSum1;
   //SD59x18 reputationAverage1=0
   int256 countDRus=0;
   ...
   SD59x18 DRis;
     ...
     if(DRusimobileuser.length!=1)
     {
          for(uint256 j=0;j<addressuser.length;j++){
              for(uint256 i=DRusimobileuser.length-1;i>0;i--)
              {
                 address _edgeserver=DRusiedgeserver[i];
                 address _mobileuser=DRusimobileuser[i];
                if(edgeserver==_edgeserver){ 
                   if(_mobileuser==addressuser[j]){
                       DRis=DRusi[_edgeserver][addressuser[j]][i];
                       //sunnn.push(DRis);
                       // =reputationSum1.add(DRis);
                      
                      reputationSum1 =reputationSum1.add(DRis);
                      countDRus++; 
                       break;   
                    }
      ...
      if(n!=0){
       ...
       NRus=(reputationSum1).div(wrap(countDRus));
